  ...
  Widget _buildMainButton(ProductsData productsData, MaterialsData materialsData) {
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: () {
          // 1. Validation for Image (التعديل الجديد هنا)
          if (_image == null) {
          ...
          // 5. Everything is valid, Proceed to Add Product
          String finalPrice = _includesMaterials 
              ? _finalPriceWithMargin.toStringAsFixed(2) 
              : _manualPriceController.text;

          productsData.addProduct(ProductModel(
            name: _productNameController.text,
            quantity: _productQtyController.text,
            price: finalPrice,
            imagePath: _image?.path, // تم التأكد من وجود الصورة سابقاً
            usedMaterials: List.from(_addedMaterials),
          ));

          // 6. Deduct Materials if applicable
          if (_includesMaterials) {
            for (var item in _addedMaterials) {
              List<String> parts = item.split(" - ");
              String matName = parts[0];
              double qtyPerUnit = double.tryParse(parts[1]) ?? 0.0;
              materialsData.deductMaterial(matName, productQtyToMake * qtyPerUnit);
            }
          }
